package org.fund.authentication.otp.generator;

import org.fund.common.FundUtils;

public class OtpPhysicalGenerator {
    private byte[] secret;
     ...
     */
    public OtpPhysicalGenerator(byte[] secret,
                      int codeDigits,
                      boolean addChecksum,
    ...
    public OtpPhysicalGenerator(String secretHex, int lookahead) {
        this.secret = FundUtils.hexStr2Bytes(secretHex);
        this.codeDigits = 6;
    ...
    public String generate(int counter) {
        ...
        // compute hmac hash
        byte[] hash = FundUtils.hmac_sha("HmacSHA1", this.secret, text);
     ...
     * @throws RuntimeException when the response was out of reach
     */
    public int validate(int counter, String otp) throws Exception {
        int i = 0;
        while (i <= lookahead) {
            if (generate(counter + i).equals(otp)) return counter + i;
            i++;
        }
        throw new RuntimeException("Provided OtpHandler response is outside the lookahead range");
    }
