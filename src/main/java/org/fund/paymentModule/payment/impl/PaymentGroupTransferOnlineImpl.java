...
@Service
public class PaymentGroupTransferOnlineImpl extends PaymentAbstract {
    public PaymentGroupTransferOnlineImpl(JpaRepository repository, VoucherService voucherService, ParamService paramService) {
    ...
    protected void validatePaymentDetailDataForInsert(List<PaymentDetail> paymentDetails) {
        ...
        if (paymentDetails.getFirst().getPayment().getPaymentOrigin().getId().equals(org.fund.paymentModule.payment.constant.PaymentOrigin.REQUEST_MANAGEMENT.getId())) {
            if (paymentDetails.stream().anyMatch(a -> FundUtils.isNull(a.getOrderId()))) {
    ...
    public Boolean accept(PaymentType paymentType) {
        return paymentType == PaymentType.GROUP_TRANSFER_ONLINE;
    }
    ...
    private List<VoucherDetail> createVoucherDetailsForPayment(PaymentDetail paymentDetail) {
        ...
        FundBankAccount fundBankAccount = repository.findAll(FundBankAccount.class).stream()
                .filter(a -> a.getFund().getId().equals(paymentDetail.getPayment().getFund().getId()) &&
                        a.getBankAccount().getId().equals(paymentDetail.getBankAccount().getId()))
                .findFirst().orElse(null);
        list.add(voucherService.createVoucherDetail(
                paymentDetail.getPayment().getFromSubsidiaryLedger(),
                fundBankAccount.getDetailLedger(),
                0L,
                paymentDetail.getAmount(),
                comments,
                paymentDetail.getId()
